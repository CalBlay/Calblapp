...
test('password helpers verify bcrypt values and matching plaintext without rehashing', async () => {
...
test('password storage helper preserves hashes and stores trimmed plaintext', async () => {
  ...
  assert.equal(await preparePasswordForStorage(' new-secret '), 'new-secret')
})
