...
import { filterClientNames } from '@/services/spaces/zohoClients'
...
export async function GET(request: Request): Promise<Response> {
  try {
    ...
    const snap = await db.collection(SPACES_ZOHO_CLIENTS_COLLECTION).get()
    const names = snap.docs
      .map((doc) => String(doc.data().nom || '').trim())
      .filter(Boolean)

    return NextResponse.json(
      { data: filterClientNames(names, q) },
      { status: 200 }
    )
  } catch (err) {
