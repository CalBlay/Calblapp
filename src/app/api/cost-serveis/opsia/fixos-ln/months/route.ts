...
export async function GET(req: NextRequest) {
  ...
  try {
    const docs = await listOpsiaFixedLnMonthDocs({ fromYm: from, toYm: to })
    const rows = flattenFixedLnMonthsToRows(docs)
    ...
    console.error('[cost-serveis/opsia/fixos-ln/months]', err)
    return NextResponse.json(
