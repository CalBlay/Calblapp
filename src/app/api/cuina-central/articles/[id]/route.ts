...
export async function PATCH(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  if (body?.unit !== undefined) patch.unit = cleanText(body.unit)
  if (body?.packagingLabel !== undefined) patch.packagingLabel = cleanText(body.packagingLabel)
  if (body?.packagingQty !== undefined) {
    patch.packagingQty = body.packagingQty == null ? null : Number(body.packagingQty)
  }
