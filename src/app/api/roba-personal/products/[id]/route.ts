...
export async function PATCH(
  ...
  const nextSize = patch.size !== undefined ? str(patch.size) : str(cur.size)
  if (!nextCode || !nextSupplier || !nextName || !nextSize) {
    return NextResponse.json(
      { error: 'code, supplier, name i size són obligatoris i no poden quedar buits.' },
      { status: 400 }
    )
  }
