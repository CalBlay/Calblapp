...
export default function CuinaGroupRoleLineRow({
  line,
  assignment,
  mode,
  responsables,
    ...
    <div className="overflow-x-auto rounded-md border border-slate-200 bg-slate-50/70 px-2 py-1.5">
      <div className="flex min-w-max items-center gap-1.5">
        ...
        <select
          ...
          disabled={isCenterExternalExtra}
        ...
        {isCenterExternalExtra && mode === 'manual' ? (
          <Input
            value={line.personName || getExternalWorkerBaseLabel('centerExternalExtra')}
            onChange={(e) =>
              onLinePatch({
                personId: '',
                personName: normalizeExternalWorkerName({
            ...
            className={`${fieldClass} min-w-[14rem] w-[16rem] shrink-0 px-2`}
            placeholder={`Nom ${getExternalWorkerBaseLabel('centerExternalExtra')}`}
            aria-label="Nom extra centre extern"
          />
        ) : (
        <select
          value={personValue}
        ...
        <Input
          ...
          disabled={isCenterExternalExtra}
          className={`${fieldClass} w-[5.35rem] shrink-0 px-1.5 tabular-nums ${isCenterExternalExtra ? 'opacity-50' : ''}`}
