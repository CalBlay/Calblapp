...
import { CheckCircle2, ChevronDown, ChevronUp, ClipboardList, X } from 'lucide-react'
...
export default function SurveyLaunchPanel({
  ...
  const surveyLaunched = Boolean(latestSurvey)
      ...
      <>
        <div className="flex justify-end">
          <button
            ...
            aria-label="Sondeig de disponibilitat"
  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-slate-50/70 p-4 space-y-3">
      ...
      >
        ...
        <div className="flex items-start gap-3">
          ...
          {compactTrigger ? (
            <button
              type="button"
              className="rounded-full p-1 text-slate-400 hover:bg-slate-200/70 hover:text-slate-700"
              aria-label="Tancar sondeig"
              onClick={(e) => {
                e.stopPropagation()
                setPanelOpen(false)
              }}
      ...
      {(compactTrigger || panelOpen) ? panelBody : null}
      {responsesDialog}
    </div>
  )
}
