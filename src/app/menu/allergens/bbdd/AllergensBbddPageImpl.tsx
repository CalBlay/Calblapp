...
export default function AllergensBbddPage() {
  ...
  const [pendingImportMode, setPendingImportMode] = useState<'replace' | 'incremental' | null>(null)
  const importFileRef = useRef<HTMLInputElement | null>(null)
  const importModeRef = useRef<'replace' | 'incremental' | null>(null)
  ...
  const handleDeleteImportConflict = async (conflictId: string) => {
    setLoading(true)
    setStatus('')
    ...
    try {
      ...
      console.error(err)
      setStatus('Error eliminant el conflicte.')
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleManualImport = async (file: File, mode: 'replace' | 'incremental') => {
    ...
    try {
      const { parsedRows, categoryMap, familyMap, menuMap } = await parseImportWorkbook(file)
      const groupedByCode = new Map<string, typeof parsedRows>()
      parsedRows.forEach(entry => {
        if (!groupedByCode.has(entry.code)) groupedByCode.set(entry.code, [])
        groupedByCode.get(entry.code)?.push(entry)
      })

      const duplicateConflicts: ImportConflictItem[] = []
      const validRows = Array.from(groupedByCode.entries()).flatMap(([code, entries]) => {
      ...
      console.error(err)
      setStatus('Error durant la importacio manual.')
    } finally {
      setLoading(false)
    ...
    plats
      .map(plat => {
        const row: Record<string, string> = {
          ...
          Nom_ES: plat.name?.es || '',
          Nom_EN: plat.name?.en || '',
          Grup: plat.familyLabel || '',
          Tipus: plat.categoryLabel || '',
  ...
  const handleExportPdf = async () => {
    ...
    try {
      ...
      pdf.setFontSize(12)
      pdf.text('BBDD Allergens i plats', margin, y)
      y += lineHeight + 6
      pdf.setFontSize(9)
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 mb-2">
            ...
            <div className="flex flex-wrap items-center justify-end gap-2">
              ...
              <Button
                ...
                onClick={() => {
                  importModeRef.current = 'incremental'
              ...
              <input
                ...
                onChange={event => {
                  ...
                  const mode = importModeRef.current || pendingImportMode
                  if (!file || !mode) return
                  void handleManualImport(file, mode)
                }}
              />
            </div>
          </div>

          <div className="hidden mb-4 rounded-lg border border-amber-200 bg-amber-50 p-3">
            ...
            <p className="mt-2 text-xs text-slate-600">
              Importacio incremental: si un codi ja existeix, et preguntara si el vols actualitzar.
              Reemplaçar base: elimina la base actual i carrega el contingut de l'Excel.
            </p>
          </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
              <div>
                ...
                >
                  ...
                  <SelectContent>
                    <SelectItem value={EMPTY_SELECT}>Sense grup</SelectItem>
                    {families.map(fam => (
              ...
              <div>
                ...
                >
                  ...
                  <SelectContent>
                    <SelectItem value={EMPTY_SELECT}>Sense tipus</SelectItem>
                    {categories.map(cat => (
        ...
        {importConflicts.length > 0 && (
          <div className="bg-white border border-rose-200 rounded-xl p-5 shadow-sm">
            <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
              ...
              <p className="text-sm text-rose-700">{importConflicts.length} pendents</p>
            </div>
              ...
              {importConflicts.map(conflict => (
                ...
                >
                  <div className="flex flex-wrap items-start justify-between gap-3">
                        ...
                        <div className="mt-2 flex flex-col gap-1">
                          {conflict.entries.map((entry, index) => (
                            <p key={`${conflict.id}-${index}`} className="text-xs text-slate-600">
                              Codi {entry.code || conflict.code || '-'} · {entry.sheet || 'sense full'} · fila{' '}
                              {entry.row || '-'} · {entry.nameCa || 'sense nom'}
                            </p>
                          ))}
                        </div>
                      )}
                    ...
                    <Button
                      variant="outline"
                      onClick={() => void handleDeleteImportConflict(conflict.id)}
                      disabled={loading}
                    >
                      Eliminar conflicte
                    </Button>
