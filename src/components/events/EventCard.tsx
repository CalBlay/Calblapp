...
import { MapPin, Users, Tag, Info, Clock, MessageCircle, UserRound, ClipboardList, FileText } from 'lucide-react'
...
export default function EventCard({
  ...
  const documentsButton = showDocuments && onOpenDocuments ? (
    <button
      type="button"
      aria-label={
        event.hasDocuments
          ? `Veure documents adjunts de ${displaySummary}`
          : `Sense documents adjunts de ${displaySummary}`
      }
      title={event.hasDocuments ? 'Veure documents adjunts' : 'Sense documents adjunts'}
      className={cn(
      ...
      >
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0 flex-1 space-y-1">
            <div className="flex items-start justify-between gap-2">
              <h3
                className="min-w-0 flex-1 text-[13px] font-semibold leading-snug text-gray-900 line-clamp-2"
                title={displaySummary}
              >
                {displaySummary}
              </h3>
  ...
  return (
    ...
    >
      <div className="flex min-h-[2.75rem] items-start justify-between gap-2 sm:min-h-0">
        <h3
          className="line-clamp-2 min-w-0 flex-1 text-[15px] font-semibold leading-snug text-slate-800 sm:line-clamp-1 sm:text-base lg:text-[17px]"
          title={displaySummary}
        >
          {displaySummary}
        </h3>
